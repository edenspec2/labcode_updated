...
def plot_interactions(xyz_df, color, dipole_df=None, origin=None,sterimol_params=None):
    ...
    def dipole_trace():
        ...
        try:
            ...
            scale=2
            comps = [
                (((vec[0]+1)*scale, 0, 0), 'red', 'Dipole X'),
                ((0, (vec[1]+1)*scale, 0), 'green', 'Dipole Y'),
                ((0, 0, (vec[2]+1)*scale), 'blue', 'Dipole Z'),
                (((vec[0]+1)*scale, (vec[1]+1)*scale, (vec[2]+1)*scale), 'purple', 'Total Dipole')
            ]
