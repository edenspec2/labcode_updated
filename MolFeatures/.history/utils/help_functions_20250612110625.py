...
def extract_connectivity(xyz_df, threshhold_distance=1.82):
    print("Starting extract_connectivity function")
    coordinates=np.array(xyz_df[['x','y','z']].values)
    atoms_symbol=np.array(xyz_df['atom'].values)
    print(f"Number of atoms: {len(atoms_symbol)}")
    ...
    print(f"Initial distance matrix shape: {dist_df.shape}")
    ...
    print(f"Processing distance matrix with threshhold_distance={threshhold_distance}")
    for idx, row in enumerate(dist_array):
        ...
        # Special condition for Cl, Br, F, I atoms
    ...
    print(f"Removing {len(remove_list)} entries based on initial criteria")
    dist_df=dist_df.drop(remove_list)
    print(f"Distance matrix shape after initial filtering: {dist_df.shape}")
    ...
    print(f"Distance matrix shape after removing duplicates: {dist_df.shape}")
    
    # Handle special atoms to keep only the shortest bond for each
    special_atoms_idxs = {}
    for idx, row in dist_df.iterrows():
        if row['first_atom'] in special_atoms:
            atom_idx = row[0]
            if atom_idx not in special_atoms_idxs:
                special_atoms_idxs[atom_idx] = []
            special_atoms_idxs[atom_idx].append((idx, row['value']))
    ...
    # For each special atom, keep only the shortest bond
    special_atoms_to_remove = []
    for atom_idx, bonds in special_atoms_idxs.items():
        if len(bonds) > 1:
            print(f"Special atom {atom_idx} ({atoms_symbol[atom_idx]}) has {len(bonds)} bonds")
            # Sort by distance (value) and keep only the shortest
            bonds.sort(key=lambda x: x[1])
            # Add all except the shortest bond to removal list
            special_atoms_to_remove.extend([idx for idx, _ in bonds[1:]])
    
    print(f"Removing {len(special_atoms_to_remove)} bonds for special atoms")
    dist_df = dist_df.drop(special_atoms_to_remove)
    print(f"Distance matrix shape after special atom filtering: {dist_df.shape}")
    
    # Handle Pd atoms separately
    ...
    try:
        for pd_idx, group in pd_bonds.groupby('pd_idx'):
            print(f"Pd atom {pd_idx} has {len(group)} bonds, keeping the 2 shortest")
            shortest = group.nsmallest(2, 'value').index
            kept_pd.extend(shortest)
    except Exception as e:
        print(f"Error processing Pd atoms: {e}")
    ...
    print(f"Final connectivity matrix shape: {final.shape}")
