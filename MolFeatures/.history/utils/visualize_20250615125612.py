...
def plot_interactions(xyz_df, color, dipole_df=None, origin=None,sterimol_params=None):
    ...
    def dipole_trace():
        """
        Return a list of 3D Plotly traces for dipole vector components (X, Y, Z, Total).
        Each vector is represented by a shaft (line) and a head (cone).
        """
        traces = []
        ...
        try:
            # Extract the dipole vector (x, y, z), dropping the 'total' column if present
            vec = dipole_df.iloc[0].to_numpy(dtype=float)[:-1]

            # Use center-of-mass or provided origin for tail position
            ...
            scale=2
            print(f"dipole vector: {vec}")
            comps = [
                (((vec[0]+0)*scale, 0, 0), 'red', 'Dipole X'),
                ((0, (vec[1]+0)*scale, 0), 'green', 'Dipole Y'),
                ((0, 0, (vec[2]+0)*scale), 'blue', 'Dipole Z'),
                (((vec[0]+0)*scale, (vec[1]+0)*scale, (vec[2]+0)*scale), 'purple', 'Total Dipole')
            ]

            for (u, v, w), color, label in comps:
                length = np.linalg.norm([u, v, w])
                if length < 1e-8:
                    continue

                # Define arrow endpoint and shaft size
                end = np.array([x0 + u, y0 + v, z0 + w])
                shaft_end = end - 0.15 * (end - tail) / length

                # Add shaft (thicker line)
                traces.append(go.Scatter3d(
                    x=[x0, shaft_end[0]],
                    y=[y0, shaft_end[1]],
                    z=[z0, shaft_end[2]],
                    mode='lines',
                    line=dict(color=color, width=8),  # thicker line
                    name=label,
                    showlegend=True
                ))

                # Add cone head (larger and better-scaled)
                traces.append(go.Cone(
                    x=[end[0]], y=[end[1]], z=[end[2]],
                    u=[u], v=[v], w=[w],
                    anchor='tip',
                    sizemode='scaled',
                    sizeref=0.2,  # control cone size
                    showscale=False,
