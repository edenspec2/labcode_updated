...
def extract_connectivity(xyz_df, threshhold_distance=1.82):
    ...
    keep_indices = set(dist_df.index)  # Start with everything that's left

    for atom_type in special_atoms:
        # Find all rows where this atom is first or second atom
        mask = (dist_df['first_atom'] == atom_type) | (dist_df['second_atom'] == atom_type)
        special_bonds = dist_df[mask]

        # If there are any, keep only the shortest for each unique special atom index
        for idx in set(list(special_bonds['a1']) + list(special_bonds['a2'])):
            atom_is_special = (atoms_symbol[idx] == atom_type)
            if atom_is_special:
                mask_atom = ((special_bonds['a1'] == idx) | (special_bonds['a2'] == idx))
                candidate_bonds = special_bonds[mask_atom]
                if not candidate_bonds.empty:
                    shortest_idx = candidate_bonds['value'].idxmin()
                    # Remove all other bonds for this special atom
                    remove_others = set(candidate_bonds.index) - {shortest_idx}
                    keep_indices -= remove_others
