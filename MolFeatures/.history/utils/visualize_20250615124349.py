...
def plot_interactions(xyz_df, color, dipole_df=None, origin=None,sterimol_params=None):
    ...
    def dipole_trace():
        ...
        try:
            ...
            # Define the individual X, Y, Z components with appropriate colors
            comps = [
                ((vec[0], 0, 0), 'red', 'X'),      # X component (red)
                ((0, vec[1], 0), 'green', 'Y'),    # Y component (green)  
                ((0, 0, vec[2]), 'blue', 'Z'),     # Z component (blue)
                ((vec[0], vec[1], vec[2]), 'purple', 'Total')  # Total vector (purple)
            ]
