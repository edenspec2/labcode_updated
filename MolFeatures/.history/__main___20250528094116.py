...
class MoleculeApp:
    ...
    def build_question_interface(self, parent, questions, loaded_entries=None):
        ...
        for i, question in enumerate(questions):
            ...
            if question.startswith("Stretch Vibration atoms"):
                ...
                entry_widgets["Stretch_Threshold"] = stretch_threshold_entry

                # Load threshold if present, else default
                if "Stretch_Threshold" in loaded_entries:
                    stretch_threshold_entry.delete(0, 'end')
                    stretch_threshold_entry.insert(0, loaded_entries["Stretch_Threshold"])
                else:
            ...
            if question.startswith("Bending Vibration atoms"):
                ...
                entry_widgets["Bend_Threshold"] = bend_threshold_entry

                # Load threshold if present, else default
                if "Bend Threshold" in loaded_entries:
                    bend_threshold_entry.delete(0, 'end')
                    bend_threshold_entry.insert(0, loaded_entries["Bend_Threshold"])
                else:
    ...
    def open_question_window(self):
        ...
        questions = [
                "Ring Vibration atoms - by order -> Pick primary atom and para to it: \n example: 13,17",
                "Stretch Vibration atoms- enter bonded atom pairs: \n example: 1,2 4,5",
                "Strech_Threshold - default is 1600 for carbonyl stretch",
                "Bending Vibration atoms - enter atom pairs that have a common atom: \n example: 4,7",
                "Bending_Threshold - default is 1600 ",
                "Dipole atoms - indices for coordination transformation: \n example: 4,5,6 - origin, y-axis, new xy plane",
                "Center Atoms - indices to move Geometric Center: \n example 1,2,3,4,5,6 - move to Ring Center",
                "NPA manipulation atoms - Insert atoms to show NPA: \n example: 1,2,4",
                "charge values - Insert atoms to show charge: \n example: 1,2,3,4",
