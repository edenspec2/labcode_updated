...
class Molecules():
    ...
    def get_molecules_comp_set_app(self, entry_widgets, parameters=None, answers_list=None, save_as=False):
        ...
        iso = parameters.get('Isotropic', parameters.get('Isotropic', False))
            ...
            try:
                res_df = pd.concat([
                    ...
                    dict_to_horizontal_df(
                        self.get_stretch_vibration_dict(answers['stretch'], answers['stretch threshold'][0])
                    )
                ], axis=1)
            except Exception:
        ...
        if answers['bending']:
            try:
                res_df = pd.concat([
                    res_df,
                    dict_to_horizontal_df(
                        self.get_bend_vibration_dict(answers['bending'], answers['bend threshold'][0])
                    )
                ], axis=1)
            except Exception:
