...
def get_b1s_list(extended_df, scans=90//5,plot_result=False):
    ...
    for idx, row in extended_df.iterrows():
        circle_points = generate_circle(row['x'], row['z'], row['radius'], n_points=20)
    ...
    b1s, b1s_loc, b1_planes = [], [], []
    sterimol_df=b1s_for_loop_function(extended_df, b1s, b1s_loc, list(degree_list), plane, b1_planes)
...
class Molecule():
    ...
    def get_charge_df(self, atoms_indices: List[int], 
        ...
        if isinstance(type, str):
            if type.lower() == 'all':
                return {t: extract_charge(df) for t, df in self.charge_dict.items()}
            else:
    ...
    def get_charge_diff_df(self, diff_indices: Union[List[List[int]], List[int]], 
        ...
        if isinstance(type, str):
            if type.lower() == 'all':
                return {t: compute_diff(df) for t, df in self.charge_dict.items()}
            else:
                if type not in self.charge_dict:
                    raise ValueError(f"Charge type '{type}' is not available in charge_dict.")
    ...
    def get_npa_df_single(self, atoms: List[int], sub_atoms: Optional[List[int]] = None, type='nbo') -> pd.DataFrame:
        ...
        df_trans = self.get_coordination_transformation_df(atoms)
    ...
    def get_molecules_comp_set_app(self,answers_dict: dict,
        ...
        if answers_list[7] and answers_list[7]!= []:
            try:
                ...
                print(help_functions.dict_to_horizontal_df(self.get_dipole_dict(answers_list[7])))
            except Exception as e:
                print(e)
                pass
        ...
        if answers_list[8] and answers_list[8]!= []:
            try:    
                res_df=pd.concat([res_df,help_functions.dict_to_horizontal_df(self.get_charge_df_dict(answers_list[8]))],axis=1)
            except Exception as e:
                print(e)
                pass
