...
def process_gaussian_standard_orientation_text(log_text: str) -> pd.DataFrame:
    ...
    df['atom'] = df['atom'].astype(int).replace(GeneralConstants.ATOMIC_NUMBERS.value , inplace=True)
    print(f"Standard orientation DataFrame created with {n_atoms} atoms.",df['atom'])
