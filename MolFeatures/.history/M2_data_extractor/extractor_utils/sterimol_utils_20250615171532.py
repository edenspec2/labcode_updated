...
def get_extended_df_for_sterimol(coordinates_df, bonds_df, radii='CPK'):
    """
    Adds steric-related information to the coordinates DataFrame.

    Parameters
    ----------
    coordinates_df : pd.DataFrame
        DataFrame with columns ['x', 'y', 'z', 'atom']
    bonds_df : pd.DataFrame
        DataFrame of molecular bonds
    radii : str, optional
        The type of atomic radii to use ('bondi' or 'CPK'), by default 'CPK'

    Returns
    -------
    pd.DataFrame
        The extended DataFrame with columns: 'atype', 'magnitude', 'radius', 'B5', 'L'
    """

    print(f"🔧 Radii type selected: {radii}")
    radii_map = GeneralConstants.CPK_RADII.value if radii == 'CPK' else GeneralConstants.BONDI_RADII.value

    df = coordinates_df.copy()
    print("📋 Initial coordinates_df:")
    print(df.head())

    if radii == 'bondi':
        df['atype'] = df['atom']
        print("🔗 Using atom symbols directly for 'atype'")
    else:
        df['atype'] = nob_atype(coordinates_df, bonds_df)
        print("🔬 Inferred 'atype' using nob_atype()")

    print("🧪 Atom types (atype):")
    print(df['atype'].unique())

    df['magnitude'] = np.linalg.norm(df[['x', 'z']].astype(float), axis=1)
    print("📐 Computed magnitudes in XZ plane:")
    print(df['magnitude'].head())

    df['radius'] = df['atype'].map(radii_map)
    print("⚛️ Mapped atomic radii:")
    print(df[['atype', 'radius']].drop_duplicates().head())

    df['B5'] = df['radius'] + df['magnitude']
    df['L'] = df['y'] + df['radius']

    print("✅ Final extended DataFrame:")
    print(df[['atom', 'atype', 'x', 'y', 'z', 'magnitude', 'radius', 'B5', 'L']].head())
