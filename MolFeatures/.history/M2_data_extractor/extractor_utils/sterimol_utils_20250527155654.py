...
def get_center_of_mass(xyz_df):
    ...
    masses=np.array([help_functions.help_functions.GeneralConstants.ATOMIC_WEIGHTS.value[symbol] for symbol in atoms_symbol])
...
def get_extended_df_for_sterimol(coordinates_df, bonds_df, radii='CPK'):
    ...
    bond_type_map_regular = help_functions.GeneralConstants.REGULAR_BOND_TYPE.value
    bond_type_map=help_functions.GeneralConstants.BOND_TYPE.value
    ## if radius is cpk mapping should be done on atype, else on atom
    radii_map = help_functions.GeneralConstants.CPK_RADII.value if radii == 'CPK' else help_functions.GeneralConstants.BONDI_RADII.value
    ...
    if radii == 'bondi':
        ...
        df['atype']=help_functions.nob_atype(coordinates_df, bonds_df)

    df['magnitude'] = magnitude = np.linalg.norm((df[['x', 'z']].astype(float)), axis=1)
