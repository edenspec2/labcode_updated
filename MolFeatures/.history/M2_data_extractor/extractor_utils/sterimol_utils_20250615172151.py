...
def get_molecule_connections(bonds_df, source, direction):
    """
    Finds the unique atom indices involved in all simple paths from source to direction
    in a molecular graph defined by bonds_df.

    Parameters
    ----------
    bonds_df : pd.DataFrame
        DataFrame of bonds (two columns of atom indices, 1-based).
    source : int
        Source atom index (0-based).
    direction : int
        Target atom index (0-based).

    Returns
    -------
    np.array
        Unique list of atom indices in all paths from source to direction.
    """
    print("🔍 get_molecule_connections called")
    print(f"📊 Input bonds_df shape: {bonds_df.shape}")
    print(f"🔢 Source: {source}, Direction: {direction}")

    try:
        # Ensure correct formatting
        bonds_df.columns = [0, 1]
        bonds_df[[0, 1]] = bonds_df[[0, 1]].apply(pd.to_numeric, errors='raise').astype(int)
        print(f"✅ bonds_df after coercion:\n{bonds_df.head()}")
    except Exception as e:
        print(f"❌ Error preparing bonds_df: {e}")
        raise

    try:
        graph = ig.Graph.DataFrame(edges=bonds_df, directed=False)
        print(f"📈 Graph created: {graph.vcount()} nodes, {graph.ecount()} edges")
    except Exception as e:
        print(f"❌ Failed to create igraph graph: {e}")
        raise

    try:
        paths = graph.get_all_simple_paths(v=source, mode='ALL')
        print(f"🔗 Found {len(paths)} paths from {source} to other nodes")
        with_direction = [path for path in paths if direction in path]
        print(f"🎯 Paths containing direction {direction}: {len(with_direction)}")
    except Exception as e:
        print(f"❌ Error finding paths: {e}")
        raise

    if not with_direction:
        print(f"⚠️ No paths from {source} to {direction}. Returning empty array.")
        return np.array([])

    # Flatten and deduplicate all atoms in the paths
    longest_path = np.unique(flatten_list(with_direction))
    print(f"🧬 Unique atoms in paths from {source} to {direction}:\n{longest_path}")
