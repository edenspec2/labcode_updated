    ...
    def get_dipole_gaussian_df_single(self, atoms,origin ,visualize_bool=False) -> pd.DataFrame:
        ...
        if visualize_bool:
            if atoms:   
                ...
                coords = self.coordinates_array[atoms]
                xyz_df = self.get_coordination_transformation_df(self.xyz_df, atoms)
                print(xyz_df)
                visualize.show_single_molecule(molecule_name=self.molecule_name, xyz_df=xyz_df, dipole_df=dipole_df,origin=coords)
        return dipole_df
