...
def process_gaussian_standard_orientation_text(log_text: str) -> pd.DataFrame:
    ...
    df['atom'] = df['atom'].replace(GeneralConstants.ATOMIC_NUMBERS.value)
...
def gauss_file_handler(gauss_filename, export=False):
    ...
    try:
        ...
        if not standard_orientation_df.empty and not ev_df.empty:
            # Extract the eV value (assuming the value is in the first cell of ev_df)
            ev_value = ev_df.iat[0, 0] if not ev_df.empty else None

            # Insert the eV value into the second row (index 1) of the first column of standard_orientation_df
            # If the DataFrame has less than 2 rows, extend it
            if len(standard_orientation_df) < 2:
                # Add empty rows as needed
                for _ in range(2 - len(standard_orientation_df)):
                    standard_orientation_df.loc[len(standard_orientation_df)] = [None] * standard_orientation_df.shape[1]

            standard_orientation_df.iat[1, 0] = ev_value
