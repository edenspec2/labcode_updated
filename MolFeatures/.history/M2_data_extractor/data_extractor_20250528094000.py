...
class Molecules():
    ...
    def get_molecules_comp_set_app(self, entry_widgets, parameters, answers_list=None, save_as=False):
        """
        ...
        radii = parameters.get('Radii', self.parameters.get('Radii'))
        iso = parameters.get('Isotropic', self.parameters.get('Isotropic', False))

        # Convert all answer values to list/nested list if not empty
            ...
            try:
                res_df = pd.concat([
                    ...
                    dict_to_horizontal_df(
                        self.get_stretch_vibration_dict(answers['stretch'], answers['stretch_threshold'][0])
                    )
                ], axis=1)
            except Exception:
        ...
        if answers['bending']:
            try:
                res_df = pd.concat([
                    res_df,
                    dict_to_horizontal_df(
                        self.get_bend_vibration_dict(answers['bending'], answers['bend_threshold'][0])
                    )
                ], axis=1)
            except Exception:
            ...
            try:
                res_df = pd.concat([
                    ...
                    charge_dict_to_horizontal_df(
                        self.get_charge_df_dict(answers['charge'])
                    )
                ], axis=1)
            except Exception:
        ...
        if answers['bond']:
            try:
                res_df = pd.concat([
                    res_df,
                    dict_to_horizontal_df(
                        self.get_bond_length_dict(answers['bond'])
                    )
                ], axis=1)
            except Exception:
        ...
        if iso:
            polarizability_df_concat = pd.DataFrame()
            for molecule in self.molecules:
                info = molecule.polarizability_df.rename(index={0: molecule.molecule_name})
                polarizability_df_concat = pd.concat([polarizability_df_concat, info], axis=0)
    ...
    #                 res_df,
    #                 charge_dict_to_horizontal_df(
    #                     self.get_charge_df_dict(answers_list[9])
    #                 )
    #             ], axis=1)
    #             if res_df.empty:
    #                 try:
    ...
    #                 res_df,
    #                 charge_dict_to_horizontal_df(
    #                      self.get_charge_diff_df_dict(answers_list[10])                    )
    #             ], axis=1)
    #             if res_df.empty:
    #                 try:
    #                     res_df = pd.concat([
    ...
    #                 res_df,
    #                 dict_to_horizontal_df(
    #                     self.get_sterimol_dict(answers_list[11], radii=radii)
    #                 )
    #             ], axis=1)
    #         except Exception:
    #             log_exception("get_molecules_comp_set_app – sterimol")
    ...
    #                 res_df,
    #                 dict_to_horizontal_df(
    #                     self.get_bond_length_dict(answers_list[12])
    #                 )
    #             ], axis=1)
    #         except Exception:
    #             log_exception("get_molecules_comp_set_app – bond length")
    ...
    #     # Bond angles
    #     if len(answers_list) > 13 and answers_list[13]:
    #         try:
    #             res_df = pd.concat([
    #                 res_df,
    #                 dict_to_horizontal_df(
    #                     self.get_bond_angle_dict(answers_list[13])
    #                 )
    #             ], axis=1)
    #         except Exception:
