...
class Molecules():
    ...
    def get_molecules_comp_set_app(self, entry_widgets, parameters=None, answers_list=None, save_as=False):
        ...
        for param_name, entry in entry_widgets.items():
            print(f'Processing parameter: {param_name} with entry: {entry}')
        ...
        print(f'get_molecules_comp_set_app called with before: {answers}')
        ...
        res_df = pd.DataFrame()
        print(f'get_molecules_comp_set_app called with answers: {answers}')
        if answers['ring']:
            try:
        ...
        if iso:
            ...
            for molecule in self.molecules:
                ...
                eV= molecule.energy_value
                polarizability_df_concat = pd.concat([polarizability_df_concat, info, eV], axis=0)
