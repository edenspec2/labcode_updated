...
class LinearRegressionModel:
    ...
    def predict(self, X, calc_covariance_matrix=False,confidence_level=0.95):
        ...
        # Make predictions
        # predicitons fail add debugging prints of shapes and matrices
        print(f'X_b shape: {X_b.shape} , X_b {X_b}')
        print(f'theta shape: {self.theta.shape}, theta {self.theta}')
    ...
    def predict(self, X, calc_covariance_matrix=False, confidence_level=0.95):
        ...
        """
        # If X is a DataFrame, select only the training features.
        if isinstance(X, pd.DataFrame):
            if hasattr(self, 'feature_names'):
                try:
                    X = X[self.feature_names].values
                except KeyError as e:
                    raise KeyError(f"Missing one or more training features in input X: {e}")
            else:
                X = X.values
        else:
            X = np.atleast_2d(X)
        
        # Check that the number of columns in X matches the expected number.
        expected_features = self.theta.shape[0] - 1  # subtract one for the intercept
        if X.shape[1] != expected_features:
            raise ValueError(f"Expected input X to have {expected_features} features, but got {X.shape[1]}.")
        
        # Add the intercept (bias) column to X.
        X_b = np.c_[np.ones((X.shape[0], 1)), X]
        
        # Debug prints to trace shapes and matrices.
        print(f'X_b shape: {X_b.shape} , X_b: {X_b}')
        print(f'theta shape: {self.theta.shape}, theta: {self.theta}')
        
        # Compute predictions and ensure they are array-like.
        predictions = np.atleast_1d(X_b.dot(self.theta))
        
        if calc_covariance_matrix:
            # Build the training design matrix from stored training features.
            if hasattr(self, 'feature_names'):
                X_train = self.features_df[self.feature_names].values
            else:
                X_train = np.atleast_2d(self.features_df)
            ...
            # Get the t-value for the given confidence level.
            t_value = t.ppf(1 - (1 - confidence_level) / 2, df=n_train - p)
            
            # Pre-compute the inverse of (X_train_b.T @ X_train_b) for prediction variance.
            inv_xtx = np.linalg.inv(X_train_b.T.dot(X_train_b))
            
            # Compute variance terms for each prediction.
            variance_terms = np.array([
                np.sqrt(self.residual_variance * (1 + x_i.T.dot(inv_xtx).dot(x_i)))
                for x_i in X_b
            ])
            
            # Compute lower and upper prediction intervals.
            lower_bounds = predictions - t_value * variance_terms
            upper_bounds = predictions + t_value * variance_terms
