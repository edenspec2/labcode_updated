...
class LinearRegressionModel:
    ...
    def predict_for_leftout(self, X, calc_covariance_matrix=False, confidence_level=0.95):
        ...
        """
        # If X is a DataFrame and self.feature_names is available, select the training features.
        if isinstance(X, pd.DataFrame) and hasattr(self, 'feature_names'):
            try:
        ...
        # Ensure X is at least 2D (handles single sample cases)
        X = np.atleast_2d(X)
        
        # Check that the number of features in X matches the expected number (theta includes intercept)
        ...
        # Debugging prints to trace shapes and values
        ...
        if calc_covariance_matrix:
            # Build the training design matrix (assuming self.features_df is a DataFrame)
            ...
            # Pre-compute the inverse for efficiency
            inv_xtx = np.linalg.inv(X_train_b.T.dot(X_train_b))
            
            # Calculate prediction variance for each left-out sample
            variance_terms = np.array([
                np.sqrt(self.residual_variance * (1 + x_i.T.dot(inv_xtx).dot(x_i)))
                for x_i in X_b
            ])
            
            # Calculate lower and upper prediction interval bounds
            lower_bounds = predictions - t_value * variance_terms
            upper_bounds = predictions + t_value * variance_terms
...
class ClassificationModel:
    ...
    def fit_and_evaluate_combinations(self,top_n, n_jobs=-1,threshold=0.5,bool_parallel=False):
        app=self.app
        results,vif_df=[fit_and_evaluate_single_combination_classification(self,combination, threshold=threshold) for combination in tqdm(self.features_combinations, desc='Calculating combinations'),bool_parallel=bool_parallel]
