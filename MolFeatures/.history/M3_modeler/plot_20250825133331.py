...
def _safe_table(
    ...
    *,
    ...
    Truncates and wraps content, shrinks-to-fit, and falls back to monospace if needed.
    """
    ax.set_title(title, fontsize=11, fontweight="bold", pad=8)
    ax.axis("off")

    if df is None or len(df) == 0:
        ax.text(0.5, 0.5, "No data", ha="center", va="center", fontsize=10, alpha=0.6)
        return
    ...
    col_labels = [textwrap.fill(str(c), width=wrap_width) for c in df_disp.columns]

    # ---- 2) Estimate relative column widths by content length
    def _max_len(col_series, col_label):
        mx = max((len(str(v)) for v in col_series), default=1)
        mx = max(mx, len(col_label))
        # approximate wrapped lines reduce width need; downweight by sqrt(lines)
        mx = mx / math.sqrt(max(1, math.ceil(mx / max(1, wrap_width))))
        return mx

    col_lengths = np.array([_max_len(formatted.iloc[:, j], col_labels[j]) for j in range(formatted.shape[1])], dtype=float)
    # avoid zero division
    ...
    # Column width adjustment (Matplotlib supports per-column width via cell widths)
    # Normalize widths to sum ~ 1.9 (leaves some margins)
    target_total = 1.9
    ...
    for j in range(ncols):
        for (r, c), cell in tbl.get_celld().items():
            if c == j:
                # width is in axes fraction; height auto from scale/font
    ...
    if not _fits():
        ...
        # shorten long strings for preview (already wrapped, but ensure lines don’t explode)
        with pd.option_context("display.max_rows", 100, "display.max_columns", max_cols):
