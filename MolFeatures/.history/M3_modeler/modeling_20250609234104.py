...
class LinearRegressionModel:
    ...
    def predict_for_leftout(self, X, origin_x, y=None, calc_interval=False, confidence_level=0.95):
        """
        ...
        import numpy as np
        import pandas as pd
        # Optional: for adding constant
        try:
            import statsmodels.api as sm
        except ImportError:
            sm = None

        # 1) Convert X to DataFrame to ensure consistent ordering
        print(f'X tp predict{X} , shape {X.shape}')
        ...
        # 2) Reorder/fill columns if feature_names provided
        if hasattr(self, 'feature_names'):
        ...
        x_df=origin_x.copy()
        indices_fo_x_df =self.indices_predict
        # x_df = x_df.iloc([indices_fo_x_df])

        # print(x_df)
        # 3) Convert to numpy array
        X_arr = X_df.values

        # 4) Apply stored scaler if present
        if hasattr(self, 'scaler'):
            X_arr = self.scaler.transform(X_arr)

        # 5) Add constant term if model expects it (statsmodels)
        ...
        # 6) Generate predictions (and intervals if requested)
        if calc_interval:
        ...
        # 7) Compute errors if y provided
        errors = None
        if y is not None:
        ...
        # 8) Return according to requested outputs
        if calc_interval:
