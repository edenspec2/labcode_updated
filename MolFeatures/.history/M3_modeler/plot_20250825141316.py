...
def _safe_table(
    ...
    *,
    max_rows: int = 30,
    max_cols: int = 12,
    wrap_width: int = 36,        # wrap long strings to this many chars
    floatfmt: str = "{:.4g}",
    start_fontsize: int = 10,
    min_fontsize: int = 6,
    cell_scale: float = 0.9,     # initial cell scale (x,y)
    max_shrink_steps: int = 36,  # safety cap
    ...
    """
    Render df as a readable table inside a small axes (e.g., for PDFs).
    - Truncates rows/cols.
    - Wraps long text.
    - Estimates relative column widths from content length.
    - Shrinks-to-fit by reducing font size then scaling.
    - Falls back to a monospace text dump if it still doesn't fit.

    Parameters
    ----------
    ax : matplotlib.axes.Axes
        Target axes (will be cleared and used for the table).
    df : pandas.DataFrame
        Data to display.
    title : str
    ...
    Notes
    -----
    - Numeric columns are right-aligned; others left-aligned.
    - Header row is centered and bold by default.
    - Avoids using Cell.set_ha/set_va (not available); uses set_text_props instead.
    """
    ax.set_title(title, fontsize=11, fontweight="bold", pad=8)
    ax.axis("off")

    # ---- Early exits / edge cases
    if df is None:
    ...
    formatted = df_disp.applymap(_fmt_cell)
    col_labels = [textwrap.fill(str(c), width=wrap_width) if wrap_width else str(c) for c in df_disp.columns]

    # ---- 2) Estimate relative column widths by content length (downweight wrapping)
    def _max_len(col_series, col_label):
        # consider wrapped lines; downweight by sqrt(n_lines)
    ...
    target_total = 1.9  # leave margins in the axes
    widths = (rel_widths / rel_widths.sum()) * target_total
    ...
    # Numeric detection on ORIGINAL (unformatted) dtypes
    num_like = {
        j for j in range(ncols)
        if pd.api.types.is_numeric_dtype(df_disp.dtypes[j])
    }

    # Data cell alignment (avoid Cell.set_ha / set_va)
    for (r, c), cell in tbl.get_celld().items():
        if r == 0:
    ...
    # Zebra stripes (rows only, skip header row r==0)
    if zebra:
        # find how many data rows there are
        nrows = formatted.shape[0]
        for (r, c), cell in tbl.get_celld().items():
            if r > 0 and (r % 2 == 0) and r <= nrows:
                cell.set_facecolor("#F5F5F5")

    # ---- 4) Shrink-to-fit loop: reduce font first, then scale
    fig = ax.get_figure()

    def _fits():
        # draw to get a renderer; safe on non-interactive backends
        fig.canvas.draw()
    ...
    while not _fits() and steps < max_shrink_steps:
        ...
        if fs > min_fontsize:
            ...
            # small geometric scaling after hitting min font
            # slight bias to height since rows often overflow vertically first
            tbl.scale(0.97, 0.95)

    # ---- 5) Fallback to monospace dump if still overflowing
    if not _fits():
        try:
            tbl.remove()
        except Exception:
    ...
    # ---- 6) Truncation footnote (when table fits)
    notes = []
    if len(df) > max_rows:
...
def _save_top5_pdf(results: pd.DataFrame, model, pdf_path: str = "top_models_report.pdf", k: int = 5):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for rank, idx in enumerate(top_idx, start=1):
            ...
            gs = GridSpec(2, 3, figure=fig1, height_ratios=[1.1, 1.0], width_ratios=[1.3, 1.0, 1.0])

            ax_coef = fig1.add_subplot(gs[0, :])
            _safe_table(ax_coef, coef_df, title="Coefficients")

            ax_vif = fig1.add_subplot(gs[1, 0])
            _safe_table(ax_vif, vif_df, title="VIF")

            ax_cv = fig1.add_subplot(gs[1, 1])
            _safe_table(ax_cv, folds_df.round(3), title="Cross-validation metrics", max_rows=50, max_cols=10)
...
def generate_and_display_single_combination_plot(model, features, app=None):
    ...
    try:
        ...
        shap_plot = model.plot_shap_values(features, plot=True)
