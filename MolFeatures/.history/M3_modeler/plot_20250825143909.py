...
def generate_and_display_single_combination_plot(model, features, app=None):
    ...
    try:
        ...
        # Build the actual X matrix for those features (whatever your model stores)
        X_shap = getattr(model, "X_", None) or getattr(model, "X_train", None) or getattr(model, "X", None)
        if X_shap is None:
            raise ValueError("Model does not expose training X. Pass X explicitly to plot_shap_values.")
        X_shap = X_shap.loc[:, features]
        shap_plot = model.plot_shap_values(X_shap, plot=True)
