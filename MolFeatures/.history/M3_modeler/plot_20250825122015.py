...
def _safe_table(
    ...
    max_rows: int = 30,
    max_cols: int = 12,
    max_colchars: int = 40,   # wrap long strings to this width
    floatfmt: str = "{:.4g}",
    start_fontsize: int = 10,
    min_fontsize: int = 6,
    cell_padding: float = 0.6,  # scales cell height (y) and width (x)
):
    """Render df as a table that shrinks to fit and wraps/limits content.
    Falls back to a monospace preview if it can't fit.
    """
    ...
    # ---- 1) Truncate rows/cols
    df_disp = df.iloc[:max_rows, :max_cols].copy()

    # ---- 2) Format values + wrap long strings
    def _fmt(x):
        ...
        # Wrap very long cells to multiple lines
        if len(s) > max_colchars:
            s = textwrap.fill(s, width=max_colchars)
        return s

    cell_text = df_disp.applymap(_fmt).values.tolist()
    col_labels = [textwrap.fill(str(c), width=max_colchars) for c in df_disp.columns]
    ...
    fs = start_fontsize
    tbl.set_fontsize(fs)
    # Scale cells a bit for readability; we may shrink later
    tbl.scale(cell_padding, cell_padding)

    # Bold header row & alignments
    for (row, col), cell in tbl.get_celld().items():
        if row == 0:
    ...
    # ---- 4) Try to fit: shrink font/scale until the table fits in the axes bbox
    fig = ax.get_figure()
    # We’ll attempt both font shrinking and a small geometric scale
    xscale, yscale = 1.0, 1.0

    def _fits():
        fig.canvas.draw()  # ensure renderer/bboxes are up to date
        renderer = fig.canvas.get_renderer()
        tbbox = tbl.get_window_extent(renderer=renderer)
        abbox = ax.get_window_extent(renderer=renderer)
        # a slight safety margin to avoid "crossing"
        margin = 2
        return (tbbox.width <= abbox.width - margin) and (tbbox.height <= abbox.height - margin)

    attempts = 0
    while not _fits() and attempts < 30:
        attempts += 1
        if fs > min_fontsize:
            fs -= 1
            tbl.set_fontsize(fs)
        else:
            # further shrink geometry if we've hit min font
            xscale *= 0.96
            yscale *= 0.96
            tbl.scale(0.96, 0.96)

    if not _fits():
        # ---- 5) Fallback: monospace preview (top N rows/cols)
        preview = df_disp.head(10).to_string(index=False, max_colwidth=max_colchars)
        ax.text(
            0.0, 1.0,
            preview + ("\n\n… (truncated)" if (len(df) > max_rows or df.shape[1] > max_cols) else ""),
            ha="left", va="top", fontsize=9, family="monospace"
        )
        # Remove the table (if partially drawn) to avoid overlap
        tbl.remove()
        return

    # ---- 6) Add a small footnote if we truncated
    notes = []
    if len(df) > max_rows:
...
def _save_top5_pdf(results: pd.DataFrame, model, pdf_path: str = "top_models_report.pdf", k: int = 5):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for rank, idx in enumerate(top_idx, start=1):
            ...
            _safe_table(ax_cv, folds_df, title="Cross-validation metrics")
            ...
            if shap_res and isinstance(shap_res, dict):
                print(f'Debug - in shap_res: {shap_res}')
