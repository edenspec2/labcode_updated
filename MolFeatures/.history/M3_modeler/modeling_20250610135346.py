...
def fit_and_evaluate_single_combination_regression(model, combination, r2_threshold=0.85):
    ...
    ## check
    model.model.fit(X, y)
    print(f'Fitting model with features: {combination}')
    ...
    print(f'Evaluating model with features: {combination}')
    ...
    try:
        print(model.model.params.index)
...
class LinearRegressionModel:
    ...
    def predict_for_leftout(self, X, y=None, X_train=None, y_train=None,
        ...
        print(f"Predicting on left-out samples with shape: {X.shape}")

        # --- Step 2: Optional refit ---
        if X_train is not None and y_train is not None:
            print("Refitting a new model instance for prediction.")
            new_model = copy.deepcopy(self.model.__class__())  # safe way to re-init
            new_model.fit(X_train[self._trained_features], y_train)
        ...
        # --- Step 4: Add constant if needed ---
        if sm and hasattr(model_to_use, "params") and "const" in getattr(model_to_use, "params").index:
            print("Adding constant term for statsmodels compatibility XXXXX.")
