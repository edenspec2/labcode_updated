...
class LinearRegressionModel:
    ...
    def predict(self, X, calc_covariance_matrix=False, confidence_level=0.95):
        """
        Predict on left-out molecules using the features that were used in training.
        
        Args:
            X (pd.DataFrame or np.ndarray): Feature matrix for left-out molecules.
                If a DataFrame, columns will be selected based on self.feature_names.
            calc_covariance_matrix (bool): If True, also return prediction intervals.
            confidence_level (float): Confidence level for prediction intervals.
            
        Returns:
            np.ndarray or tuple: Predictions, or (predictions, lower_bounds, upper_bounds)
                                 if calc_covariance_matrix is True.
        """
        # If X is a DataFrame, select only the features used during training.
        if isinstance(X, pd.DataFrame):
            try:
                X_selected = X[self.feature_names]
            except KeyError as e:
                raise KeyError(f"One or more training features not found in input X: {e}")
            X = X_selected.values
        else:
            # If X is a numpy array, ensure it has the correct number of columns.
            if X.ndim == 1:
                X = np.atleast_2d(X)
            expected_features = len(self.feature_names)
            if X.shape[1] != expected_features:
                raise ValueError(f"Expected X to have {expected_features} features, but got {X.shape[1]}.")

        # Add bias (intercept) term
        X_b = np.c_[np.ones((X.shape[0], 1)), X]
        
        # Check that the design matrix columns match the parameters in theta.
        if X_b.shape[1] != self.theta.shape[0]:
            raise ValueError(f"Mismatch in design matrix: X_b has {X_b.shape[1]} columns but theta has {self.theta.shape[0]} elements.")
        
        # Make predictions
        predictions = X_b.dot(self.theta)
        
        if calc_covariance_matrix:
            # Build the training design matrix with intercept
            X_train_b = sm.add_constant(self.features_df[self.feature_names].values)
            
            # Calculate residuals if not already computed
            ...
            self.model_covariance_matrix = self.residual_variance * np.linalg.inv(X_train_b.T.dot(X_train_b))
            
            # t-value for the confidence level
            t_value = t.ppf(1 - (1 - confidence_level) / 2, df=n_train - p)
            
            # Pre-compute inverse for efficiency
            inv_xtx = np.linalg.inv(X_train_b.T.dot(X_train_b))
            
            # Compute variance for each left-out sample's prediction
            variance_terms = np.array([
                np.sqrt(self.residual_variance * (1 + x_i.T.dot(inv_xtx).dot(x_i)))
                for x_i in X_b
            ])
