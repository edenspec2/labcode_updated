...
class LinearRegressionModel:
    ...
    def preform_bayesian_ridge_regression(self, random_state: int = 42, verbose: bool = True):
        """
        Fit a Bayesian Ridge Regression model to the features and target.
        Stores the fitted model in self.models with key 'Bayes. Ridge'.
        ...
        verbose : bool, optional
            If True, print fit summary (default=True).
        ...
        """
        # Sanity check
        if not hasattr(self, 'features_df') or not hasattr(self, 'target_vector'):
            print("Features and target must be loaded (run process_features_csv first).")
            return
        ...
        if not hasattr(self, 'models'):
            self.models = {}
        self.models['Bayes. Ridge'] = model

        if verbose:
            n_features = X.shape[1]
            print(f"Bayesian Ridge Regression fit on {n_features} features.")
            print("Intercept:", model.intercept_)
            # Show top 5 features by absolute coefficient
            if hasattr(self, 'features_list'):
                idx = np.argsort(np.abs(model.coef_))[::-1][:5]
                print("Top features (by |coef|):")
                for i in idx:
                    print(f"  {self.features_list[i]}: {model.coef_[i]:.5f}")
    ...
    def predict_for_leftout(self, X, y=None, calc_interval=False, confidence_level=0.95):
        ...
        if calc_interval:
            preds, lower, upper = self.predict(
                X_arr, return_interval=True, cl=confidence_level
            )
        else:
