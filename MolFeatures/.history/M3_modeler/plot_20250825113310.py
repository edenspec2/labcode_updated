...
def _safe_table(ax, df: "pd.DataFrame", title: str):
    """Render df as a table with graceful fallback if empty."""
    ...
    if df is None or len(df) == 0:
        ...
        return
    try:
        _nice_table(ax, df, title=title)
    except Exception:
        ax.axis("off")
        ax.text(0.0, 1.0, df.head(10).to_string(index=False), ha="left", va="top",
