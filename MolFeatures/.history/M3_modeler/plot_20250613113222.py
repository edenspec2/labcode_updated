...
def plot_feature_vs_target(feature_values, y_values, feature_name, y_name="Target", point_labels=None, figsize=(10, 6)):
    ...
    feature_values : array-like
        Values of the feature to plot.
    y_values : array-like
        Values of the target variable.
    feature_name : str
        Name of the feature for labeling the x-axis.
    y_name : str, optional
        Name of the target variable for labeling the y-axis.
    point_labels : array-like, optional
        Labels to display next to each point. If None, no labels are shown.
    ...
    scatter = ax.scatter(feature_values, y_values, s=70, alpha=0.7, edgecolor='w', linewidth=1)
    
    # Add regression line with confidence interval
    sns.regplot(x=feature_values, y=y_values, scatter=False, ci=95, line_kws={'color':'red'}, ax=ax)
    
    # Calculate correlation coefficient
    corr_coef = np.corrcoef(feature_values, y_values)[0, 1]
    r_squared = corr_coef**2
    
    # Add labels and title
    ax.set_xlabel(feature_name, fontsize=14)
    ax.set_ylabel(y_name, fontsize=14)
    # Switch title to reflect that y_name is now on x-axis and feature_name is on y-axis
    ax.set_title(f"Relationship between {y_name} and {feature_name}\nPearson r = {corr_coef:.3f}, R² = {r_squared:.3f}", 
                fontsize=16)
    ...
    if point_labels is not None:
        ...
        for i, label in enumerate(point_labels):
            texts.append(ax.annotate(label, (feature_values[i], y_values[i]), 
