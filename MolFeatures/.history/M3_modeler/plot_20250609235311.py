...
def generate_and_display_q2_scatter_plot(model, features, app=None):
    ...
    try:
        ...
        print("Current prediction data has:", X_left.columns.tolist())
        model.feature_names_in_=features
        print("Model was trained after:", model.feature_names_in_)
    ...
    try:
        ...
        try:
            if hasattr(model, 'indices_predict'):
                # build a mask of indices to KEEP in X,y
                keep_mask = [
                    idx not in model.indices_predict
            ...
            if model.leftout_samples is not None and len(model.leftout_samples) > 0:
                ...
                if leftout_pred is not None:
                    ...
                    print(prediction_df)
                    ## calculate and predict R2 as well and print it
