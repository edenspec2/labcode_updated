...
def _safe_table(
    ...
    *,
    ...
    min_figsize: tuple | None = None,        # (w,h) inches; None -> current fig size
    max_figsize: tuple | None = (8.2, 10.5), # A4-ish portrait area inside margins
    ...
    num_like = {j for j in range(ncols) if pd.api.types.is_numeric_dtype(df_disp.dtypes[j])}
    for (r, c), cell in tbl.get_celld().items():
        if r == 0:
            continue
        if c in num_like:
