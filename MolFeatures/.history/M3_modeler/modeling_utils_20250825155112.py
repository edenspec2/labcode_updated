...
def simi_sampler(
    ...
    # ---- Plotting (optional)
    if plot:
        # BEFORE: tag statuses and show where the dropped points are (as 'dropped')
        ...
        # AFTER: remove dropped, but keep 'kept' and 'other'; dropped are gone by design
        after_df = before_df[before_df["status"] != "dropped"].copy()
        plot_similarity_scatter_with_status(
            after_df, x_col,
            title=f"Similarity ({x_col}) AFTER truncation (class={class_label}, n={len(keep_pos)})",
