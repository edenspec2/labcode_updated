...
class MoleculeApp:
    def __init__(self, master):
        ...
        # 1) Use a CTkTextbox instead of the classic Text widget for a more native look
        ...
        self.output_text.grid(
            row=0, column=1, columnspan=2, rowspan=4,
            sticky="nsew",
            padx=(10, 10),  # horizontal padding
            pady=(10, 10)   # vertical padding
        )

        # 2) Let the middle columns expand so the textbox grows with the window
        master.grid_columnconfigure(1, weight=1)
        master.grid_columnconfigure(2, weight=1)
        master.grid_rowconfigure((0,1,2,3), weight=1)

        # 3) Optionally tweak fonts/colors
        self.output_text.configure(
            font=("Consolas", 12),
            fg_color="#1f1f1f",   # dark background
