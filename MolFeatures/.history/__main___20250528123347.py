...
def get_test_pypi_version_from_sdist(package_name="MolFeatures"):
    """
    Query Test-PyPI JSON, look for a filename matching
    MolFeatures-<VERSION>.tar.gz, extract VERSION via regex,
    and return it (or 'unknown' if not found).
    """
    ...
    pattern = re.compile(rf"^{re.escape(package_name)}-(?P<ver>[\d]+(?:\.[\d]+)*)\.tar\.gz$")
