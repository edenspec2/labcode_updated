...
def get_latest_test_pypi_sdist_version(package_name="MolFeatures"):
    """
    Query Test-PyPI for the latest MolFeatures release, and ensure that
    an sdist named 'MolFeatures-<version>.tar.gz' exists.
    Returns the version string (e.g. '0.9009') or 'unknown' on failure.
    """
    ...
    # Look for exactly the tar.gz sdist
    expected = f"{package_name}-{ver}.tar.gz"
    for fileinfo in data.get("urls", []):
        if fileinfo.get("filename") == expected:
            return ver

    # If we didn’t find it, fallback
    return "unknown"

__version__ = get_latest_test_pypi_sdist_version()
...
class MoleculeApp:
    ...
    def restart_app(self):
        ...
        # If we're running as a module, __package__ will be set to "MolFeatures"
        pkg = __package__ or ""
        args = sys.argv[1:]  # everything after the script name or module flag

        if pkg:
            # Re-launch exactly as: python -m MolFeatures <original args>
            os.execv(python, [python, "-m", pkg] + args)
        else:
            # Fallback: script was run directly
    ...
    def build_question_interface(self, parent, questions, loaded_entries=None):
        ...
        for i, question in enumerate(questions):
            ...
            if question.startswith("Stretch Vibration atoms"):
