...
steRimol.cube <- function(cubefile, coordinates, only.sub = T, drop = NULL, plot = T, degrees = 90,
  ...
  B.scanner <- function(i) {
    ...
    if (any(which(avs == min(avs)) %in% c(1, 2))) {
      print('here1')
      print(tcs[(which(abs(tc[, 1]) == min(avs), arr.ind = T))[1], 2])
...
setwd('C:\\Users\\edens\\Documents\\GitHub\\LabCode\\MolFeatures\\cube_example')
print(steRimol.cube('Ad_1_a.cube','1 2',only.sub = T,drop=NULL,plot=F,degrees = 90,isovalue = 0.003))